...
11. Exercise held-open streams up to the aggregate 32-stream cap with enough Clients to respect the four-stream-per-Client limit. Require stream 33 and queue/overload cases to fail only the affected work with finite error presentation; confirm ordinary direct EC2 traffic remains independent.
