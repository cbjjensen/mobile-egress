...
The Agent reuses the Android Agent's enrollment and endpoint-migration QRs, `POST /v1/enroll`, `POST /v1/endpoint-migrations/consume`, `GET /v1/session`, binary WebSocket protocol, relay CA pinning, mTLS role, public-target policy, one-active-Agent model, and aggregate 32-stream cap. It uses a non-exportable Secure Enclave P-256 key and a Keychain access group shared by the app and packet-tunnel extension; this is deliberately different from Android's Android Keystore implementation.
...
`-UseMacBuildServer` defaults to the documented local Mac host and user. `-MacHost`, `-MacUser`, and `-SshKeyPath` are available when the network changes. The path first verifies that the ignored `.local/mac-build-server/id_ed25519` key is not tracked in its owning checkout, creates a temporary Git bundle of the clean `HEAD`, transfers only that bundle, and builds a detached disposable Mac checkout at the exact commit. It does not update an existing Mac branch/ref or copy an ad hoc source directory. For a continuation after separately recorded phases, `-SkipPortableTests` skips the Windows suites and `-MacBuildServerStartAt warnings`, `xcode`, or `test` resumes the Mac sequence at that checkpoint. These continuation options require `-UseMacBuildServer`; the default remains the complete verification sequence. See [the Mac build-server runbook](../docs/ios-build-server.md).
...
5. `xcodebuild test -workspace . -scheme MobileEgressCore -destination platform=macOS` for the standalone package workspace, without assuming a simulator model.
...
3. Disable cellular data while leaving Wi-Fi usable. Existing Agent streams must close and new relayed requests must fail with finite errors; no Wi-Fi fallback is allowed. Restore cellular and verify the Agent can reconnect.
4. Trigger controller endpoint migration. Scan the one-use migration QR and require an exact byte-for-byte CA match. Verify only the relay origin changes and the retained Secure Enclave identity/certificate continues to authenticate. A changed CA must fail closed.
5. Stop/start the tunnel, relaunch the app, and restart the device. Verify the expected persisted enrollment and identity behavior. Test the app-managed on-demand/sleep behavior with `NEOnDemandRuleConnect`, `isOnDemandEnabled`, and `disconnectOnSleep = false`; record the actual device result and do not call it Always-On VPN.
6. Exercise held-open streams up to the aggregate 32-stream cap with enough Clients to respect the four-stream-per-Client limit. Require stream 33 and queue/overload cases to fail only the affected work with finite error presentation; confirm ordinary direct EC2 traffic remains independent.
7. Verify invalid QR input, expired capability, non-public target rejection, unavailable identity, tunnel-settings failure, and runtime failure report finite user-facing errors rather than raw secrets or unbounded diagnostic text.
8. Validate the packet tunnel's intentional empty included-route configuration on the real device. This acceptance remains unverified until it succeeds on signed hardware; neither the unsigned build nor the package tests prove that Apple accepts the empty-route configuration.

The existing Task-4 evidence covers portable tests and an unsigned `iphoneos` app-plus-extension build on a disposable configured Mac clone. It does not cover signing, Archive, TestFlight upload, simulator/device runtime, physical acceptance, or empty-route acceptance.
