...
type relayServiceState string

const (
	relayServiceNotRequired      relayServiceState = "not-required"
	relayServiceNotRegistered    relayServiceState = "not-registered"
	relayServiceApprovalRequired relayServiceState = "approval-required"
	relayServiceEnabled          relayServiceState = "enabled"
	relayServiceVersionMismatch  relayServiceState = "version-mismatch"
	relayServiceUnavailable      relayServiceState = "unavailable"
)

type desktopNative interface {
	StartTray(*DesktopApp)
	StopTray()
	HideWindow(context.Context)
	ShowWindow(context.Context)
	Quit(context.Context)
}

type desktopControllerConfig struct {
	Platform          desktopPlatform
...
func newDesktopApp(ctx context.Context, config desktopControllerConfig) (*DesktopApp, error) {
	if config.Platform != platformWindows && config.Platform != platformMacOS {
		return nil, errors.New("desktop platform is required")
	}
	if config.Store == nil || config.Gateway == nil {
		return nil, errors.New("secure store and relay gateway are required")
	}
	core, err := client.NewCore(ctx, config.Store, config.Gateway)
	if err != nil {
		return nil, err
	}
	...
	if credentials, loadErr := application.cloudRepository.AccessKeys(ctx); loadErr == nil {
...
func newWailsOptions(application *DesktopApp, platform desktopPlatform) *options.App {
	result := &options.App{
	...
	if platform == platformMacOS {
		result.Mac = &mac.Options{
			Appearance: mac.NSAppearanceNameDarkAqua, WebviewIsTransparent: false, WindowIsTranslucent: false,
		}
	} else {
		result.Windows = &windows.Options{WebviewIsTransparent: false, WindowIsTranslucent: false}
	}
	return result
}
...
func (app *DesktopApp) BeginAWSIdentityCenter(startURL, ssoRegion string) (awssdk.DeviceAuthorization, error) {
	...
	app.identitySession = nil
	app.mu.Unlock()
	if runtimeContext := app.runtimeContext(); runtimeContext != nil && app.browserOpenURL != nil {
		app.browserOpenURL(runtimeContext, authorization.VerificationURL)
	}
...
func (app *DesktopApp) Quit() {
	...
	app.shutdownApp()
	if ctx := app.runtimeContext(); ctx != nil && app.native != nil {
		app.native.Quit(ctx)
	}
}

func (app *DesktopApp) desktopPlatform() desktopPlatform {
	if app.platform == platformMacOS {
		return platformMacOS
	}
	return platformWindows
}

func (app *DesktopApp) currentRelayServiceState() relayServiceState {
	if app.desktopPlatform() == platformWindows {
		return relayServiceNotRequired
	}
	if app.relayState == nil {
...
func (app *DesktopApp) showWindow(ctx context.Context) {
	if app.native != nil {
		app.native.ShowWindow(ctx)
	}
}
