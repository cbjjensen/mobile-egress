...
- **EC2 Nodes** inventories supported `us-east-1` instances, safely prepares SSM IAM, and gives a running Agent 30 seconds to refresh its new credentials. If it remains unavailable, an explicit **Restart EC2 and continue** confirmation reboots only that selected instance, waits for a fresh SSM ping, and resumes signed Client installation. Activity events distinguish unregistered, offline, stale, and ready states without including provider errors or secrets. The page also installs/updates/repairs signed Clients, shows redacted node metadata, and reveals SOCKS credentials only on copy.
