...
- **EC2 Nodes** inventories supported `us-east-1` instances, safely prepares SSM IAM, and gives a running Agent 30 seconds to refresh its new credentials. If it remains unavailable, an explicit **Restart EC2 and continue** confirmation reboots only that selected instance, waits for a fresh SSM ping, and resumes signed Client installation. Activity events distinguish unregistered, offline, stale, and ready states without including provider errors or secrets. The page also installs/updates/repairs signed Clients, shows a redacted `127.0.0.1:1081:***:***` endpoint, copies that HTTP proxy line for Refract by default, and retains a separate SOCKS5 URL action. HTTP copying remains disabled until the node reports Client `1.0.24` or later so both ordinary HTTP and HTTPS-through-CONNECT are available.
...
- EC2 service: `MobileEgressClient`, LocalSystem, auto start, authenticated SOCKS5 at `127.0.0.1:1080` and an authenticated HTTP forward/CONNECT proxy at `127.0.0.1:1081`, state `C:\ProgramData\MobileEgress\Client`. An application on that same EC2 node must explicitly opt in to one of these loopback listeners. They are not controller-host, system-wide, VPN, public, UDP, or QUIC proxies. SOCKS, ordinary HTTP, HTTPS CONNECT, active requests, and ordinary HTTP's two retained idle destination streams all share one 32-slot relay session; the Agent-wide limit is 256 streams and idle streams expire after 15 seconds. The Client retains at most 128 recently closed stream IDs for late-frame handling. Outbound relay data prefers 16 KiB frames, while valid data frames up to 32 KiB are accepted.
...
```powershell
& .\scripts\build-windows.ps1 -ReleaseVersion 1.1.0
```
