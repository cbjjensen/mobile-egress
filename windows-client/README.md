...
- **AWS Login** supports IAM Identity Center device/browser login and DPAPI-encrypted access-key fallback. The setup helper opens the `us-east-1` IAM Identity Center dashboard for beginners. If a friend only has the AWS root login, they use root only in the browser to enable IAM Identity Center, choose **Single-Region instance** in **US East (N. Virginia)**, create a user, assign that user account access, and copy the AWS access portal URL into Mobile Egress. The fallback is for an IAM user's access key, never root access keys.
