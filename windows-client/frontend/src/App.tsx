...
const emptyBridge: BridgeStatus = { tailscaleOnline: false, relayReady: false, ownerReady: false, ready: false, needsRotation: false }
...
export default function App() {
  ...
  return <main className="shell">
    ...
    {tab === 'bridge' && <section className="stack">
      ...
      <article className="card"><p className="step-label">Step 2</p><h2>Create this PC’s relay</h2><p>This installs <code>MobileEgressRelay</code> as LocalSystem, binds it to <code>127.0.0.1:8443</code>, and publishes raw TLS through Funnel. No router or firewall port is opened.</p>{bridge.publicUrl && <div className="serialline"><span>Public Funnel origin</span><code>{bridge.publicUrl}</code></div>}<div className="row actions"><span className={`pill ${bridge.relayReady ? 'on' : ''}`}>{bridge.relayReady ? 'Relay healthy' : 'Relay not ready'}</span></div>{bridge.needsRotation ? <><p className="note">Tailscale now reports a different Funnel name. Connect AWS first if nodes are managed, then rotate and scan the migration QR on Android.</p><button className="primary" onClick={() => void rotateBridge()} disabled={!!busy}>{busy === 'rotate' ? 'Updating relay and nodes…' : 'Rotate endpoint safely'}</button></> : bridge.ownerReady && !bridge.relayReady ? <button className="primary" onClick={() => void repairBridge()} disabled={!!busy}>{busy === 'relay-repair' ? 'Repairing through UAC…' : 'Repair local relay'}</button> : <button className="primary" onClick={() => void setupBridge()} disabled={!!busy || bridge.ownerReady}>{bridge.ready ? 'Local bridge ready' : busy === 'bridge' ? 'Waiting for browser / UAC…' : 'Set up local bridge'}</button>}</article>
