...
import { AgentQr, api, AWSAccount, BridgeStatus, ControllerSnapshot, DeviceAuthorization, EC2Instance, EndpointMigration, ManagedNode, SSMInstanceStatus } from './api'
...
const emptyBridge: BridgeStatus = { platform: initialPlatform, relayServiceState: initialPlatform === 'macos' ? 'not-registered' : 'not-required', tailscaleInstalled: false, checking: true, tailscaleOnline: false, funnelReady: false, relayReady: false, ownerReady: false, ready: false, needsRotation: false }
...
export default function App() {
  ...
  const [components, setComponents] = useState<ControllerSnapshot['components']>({ tailscale: {checking:true,stale:false}, helper: {checking:true,stale:false}, relay: {checking:true,stale:false}, metadata: {checking:true,stale:false} })
  ...
  async function installNode(instance: EC2Instance) {
    if (!bridge.ready) { setError('Finish local bridge setup before installing an EC2 Client.'); setTab('bridge'); return }
