...
For upcoming releases built from this source, the primary download is the self-contained `windows-client\build\release\mobile-egress-windows-<version>\MobileEgressSetup.exe`. The guarded build signs all four application executables, embeds their compressed bytes with the public metadata, then signs setup last. Require valid timestamped Authenticode on all five executables, an embedded payload matching the independently verified signed sources, and node-release manifest v2 bound to the tracked certificate and `mobile-egress-client.exe` hash. The local `payload-verification.zip` is verification evidence, never a published asset. A compatibility `mobile-egress-windows-<version>.zip` is still produced; the published v1.1.0 through v1.1.6 ZIP contracts remain immutable. This source change does not itself publish a new release. `-CodeSigningThumbprint` is only an optional equality assertion, not a way to select another signer. Any missing/invalid signature or signer mismatch makes the entire package unusable: stop, do not distribute it, and tell anyone who received it not to run it.
...
Give a friend only the signed setup from the project's official GitHub Releases source (or its original ZIP for a release through v1.1.6) and the SHA-256 certificate fingerprint from `windows-signing\release-signing-certificate.txt` through a separate trusted channel. Never separately send a CER/DER, recovery file, or verifier bundle. Under the approved relaxed convenience boundary, malicious substitution of the download source before first trust is out of scope; do not recommend mirrors, forwarded attachments, or repackaged archives.

They may double-click `MobileEgressSetup.exe` directly; the new installer needs no extraction or neighboring payload folder. For a historical ZIP release, extract the complete ZIP first. Inspecting **Properties → Digital Signatures** or using trusted system Windows PowerShell is useful but optional; never require a verifier script as the launcher. An optional PowerShell check may report `NotTrusted` on a fresh PC before setup and must report `Valid` after trust is installed, always with the exact tracked signer. On the first run, **Unknown publisher** and SmartScreen **More info → Run anyway** may still appear because self-signing does not create SmartScreen reputation; these are acceptable only when setup carries that exact signer. Disabling SmartScreen or signature checks is not supported.
