...
function Get-MobileEgressReleaseArtifacts {
    ...
    $paths = [ordered]@{
        "mobile-egress-windows-$Version.zip" = Join-Path $RepositoryRoot "windows-client\build\release\mobile-egress-windows-$Version.zip"
        'mobile-egress-client.exe' = Join-Path $RepositoryRoot 'windows-client\build\bin\mobile-egress-client.exe'
        'app-release.apk' = Join-Path $RepositoryRoot 'android\app\build\outputs\apk\release\app-release.apk'
    }
    $artifacts = foreach ($entry in $paths.GetEnumerator()) {
        if (-not (Test-Path -LiteralPath $entry.Value -PathType Leaf)) {
            throw "Required release artifact is missing: $($entry.Value)"
        }
...
function Invoke-MobileEgressRelease {
    ...
    if ($resumeArtifacts -and -not (Test-Path -LiteralPath $windowsZip -PathType Leaf)) {
    ...
    Invoke-MobileEgressRequiredPowerShellScript -Path (Join-Path $PSScriptRoot 'test-all.ps1') -Description 'Full repository gate'
    ...
    Assert-MobileEgressReleaseArtifacts -RepositoryRoot $RepositoryRoot -Version $Version
    $artifacts = Get-MobileEgressReleaseArtifacts -RepositoryRoot $RepositoryRoot -Version $Version
    foreach ($artifact in $artifacts) {
...
Invoke-MobileEgressRelease -Version $ReleaseVersion -RepositoryRoot $repositoryRoot -PublishRelease:$Publish
