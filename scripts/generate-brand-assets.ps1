...
try {
    ...
    Save-LogoPng -Source $source -Path (Join-Path $repositoryRoot 'windows-client\internal\desktop\zfnf-menu-bar.png') -Size 36 -Transparent $true -Scale 0.9
    Save-LogoIcns -Source $source -Path (Join-Path $repositoryRoot 'windows-client\macos\appicon.icns')
} finally {
    $source.Dispose()
}

Write-Host 'Generated Android, Windows, and macOS branding assets from assets\branding\zfnf-logo-source.png.'
