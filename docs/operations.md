...
Refract or another browser-style application running on a managed EC2 node opts in with that node's **Copy proxy line** value: `127.0.0.2:1081:<username>:<password>`. The listener forwards ordinary HTTP and uses CONNECT to carry end-to-end HTTPS without Mobile Egress decrypting it. SOCKS-aware software can instead use **Copy SOCKS5 URL** at `127.0.0.2:1080`. Both values work only for applications on that same EC2 node, and there is no `.1` compatibility listener. Mobile Egress does not install a controller-host or Windows system-wide proxy, change a default route, or provide VPN, public, UDP, or QUIC proxy behavior; never add an EC2 ingress rule or public listener.
...
| Refract rejects the proxy line | Refract and Client run on the same EC2 node; Client version is `1.1.1` or later; both an `http://` and an `https://` test work through `127.0.0.2:1081` | Choose **Update**, wait for the refreshed version, then **Copy proxy line** again. Use `IP:PORT:USERNAME:PASSWORD` exactly; discard stale `.1` values and do not expose port 1081 through AWS or Windows Firewall. |
