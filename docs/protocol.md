...
```json
{
  ...
  "streamId": "opaque-base64url-id-or-empty",
...
Stream IDs are non-empty, bounded, opaque base64url values for stream messages and empty for `ping`/`pong`. Peers must not assume a UUID representation or derive meaning from an ID.
