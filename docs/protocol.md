...
Every Client identity has at most 32 streams. The single Agent/relay aggregate has at most 256, with first-come admission at both boundaries. The relay Agent writer holds at most 512 controls, 256 aggregate data frames, and two data frames per stream; a relay Client writer holds 64 controls, 32 aggregate data frames, and two per stream. Relay closed-stream tombstones are capped at 1,024. Session maps, frame sizes, open timeouts, and idle timeouts are also bounded. Outbound senders prefer 16 KiB data frames and receivers accept valid data frames up to 32 KiB. Agent outbound data is scheduled fairly across ready streams. Close processing is idempotent. Data saturation closes only the affected stream with `agent_unavailable`; required-control saturation or writer failure closes the affected session.
