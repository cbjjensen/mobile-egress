...
The controller downloads Tailscale only from the official stable package origin, checks the companion SHA-256, and requires a valid Tailscale signer before UAC install. Mobile Egress uses one self-signed local publisher certificate. Before a friend trusts it, the publisher's SHA-256 fingerprint must arrive through a separately trusted channel and be compared with the signer certificate that Windows extracts from the exact setup file through **Properties → Digital Signatures**. This OS-extracted signer comparison is the pre-trust authority; a fingerprint displayed by the untrusted setup or shipped beside it is not. Setup also verifies its Authenticode integrity before elevation and binds the exact confirmed file digest across UAC, but those self-checks do not replace the independent comparison. The signed controller embeds the exact node-release URL, SHA-256, signer thumbprint, and public publisher certificate. EC2 establishes that exact public certificate as its trust anchor before accepting a Client whose digest and Authenticode certificate thumbprint match the embedded record. Local helper and relay siblings must have the same valid signer thumbprint as the running signed controller/admin helper.
