...
The controller downloads Tailscale only from the official stable package origin, checks the companion SHA-256, and requires a valid Tailscale signer before UAC install. Mobile Egress uses one self-signed local publisher certificate. Before a friend trusts it, trusted system Windows PowerShell must run `Get-AuthenticodeSignature` on the exact setup file and report signer thumbprint `85F220C1BF05A5D3A86B5DD408787EC1B122ECB7` plus Status exactly `NotTrusted` or `Valid`; all other statuses are rejected. The SHA-256 of that OS-extracted signer certificate must match the separately shared fingerprint. Windows Properties can help view the certificate but is insufficient alone; setup/ZIP-displayed values are not authority. Genuine setup locks its own file against mutation/replacement while verifying, hashing, confirming, and waiting for its child, while the child repeats digest/signature checks before trust. These genuine-code defenses cannot authenticate malicious substituted code and do not replace the independent OS check. Only child exit zero plus a digest-bound success result authorizes launch. The signed controller embeds the exact node-release URL, SHA-256, signer thumbprint, and public publisher certificate. EC2 establishes that exact public certificate as its trust anchor before accepting a Client whose digest and Authenticode certificate thumbprint match the embedded record. Local helper and relay siblings must have the same valid signer thumbprint as the running signed controller/admin helper.
