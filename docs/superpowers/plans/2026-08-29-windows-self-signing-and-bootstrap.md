...
- Before launch, trusted system Windows PowerShell runs `Get-AuthenticodeSignature` on the exact setup file and requires the tracked thumbprint plus Status exactly `NotTrusted` or `Valid`; Properties alone and setup-displayed values are insufficient. The OS-extracted certificate fingerprint is compared with the separately shared value. Genuine parent code locks its exact executable against write/delete/replacement, verifies and hashes through that lock, holds it across confirmation and elevated-child completion, and requires child exit zero plus a bound success result before launch. The child checks its own digest and exact signature before adding trust as defense in depth; genuine-code checks do not authenticate substituted malicious code.
