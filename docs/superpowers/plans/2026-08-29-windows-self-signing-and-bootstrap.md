...
- Friends receive the publisher SHA-256 fingerprint through a separate channel and can inspect the exact setup's Windows signature through Properties or trusted system Windows PowerShell before double-clicking it. The parent displays the embedded fingerprint, requires explicit confirmation, locks its exact executable through elevated-child completion, and binds the request/result to its locked digest. It launches the controller only when child exit is zero and the bound result reports success. The child checks its own digest/signature before trust as defense in depth.
