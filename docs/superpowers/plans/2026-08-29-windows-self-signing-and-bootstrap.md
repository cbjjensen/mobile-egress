...
- Before launch, the recipient compares the signer certificate fingerprint extracted from the exact setup file by Windows Properties/signature inspection with the separately shared value; a value displayed by the untrusted setup is only a reminder. The parent verifies its intact exact Authenticode signer, requires explicit confirmation, hashes the confirmed setup into a bounded nonce-bound request, requests UAC only for that same setup path, requires a successful child exit plus bound result, then launches the installed controller unelevated. The child checks its own digest and exact signature before adding trust.
