...
- Before launch, a command copied from separately shared instructions into trusted system Windows PowerShell holds a mutation-denying setup handle across exact Authenticode Status/SHA-1/certificate-SHA-256 checks, hashes that same handle, and calls `Start-Process` with `--verified-setup-sha256`. Direct launch is rejected. Genuine parent code locks its exact executable, requires the locked digest to equal that argument, retains the lock through elevated-child completion, and requires child exit zero plus a bound success result before controller launch. The child checks its own digest/signature before trust as defense in depth.
