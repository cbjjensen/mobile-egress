# Authenticated paced stream acceptance

This runbook defines a future authorized run of the non-release `capacityharness` build: one legitimate Client identity opens, verifies, and holds 512 streams by default, then a second legitimate identity opens and verifies one additional stream while every holder stream stays open. After a 15-minute hold, one holder stream closes and the holder opens and verifies a replacement. Production imposes no fixed active-stream ceiling; this finite developer run is not a throughput benchmark and must never target a production relay.

**Status: `PENDING` / definition only.** Authenticated load, soak, memory, and physical-device acceptance remain unexecuted. Deterministic unit/component tests and native builds do not establish physical capacity or browser throughput. Retain these commands for a separately authorized acceptance event.

The runner accepts `--streams=N` (1?4096, default 512) and `--open-interval=D` (1 ms?1 s, default 10 ms). Each holder stream finishes opening and verification before the delay and next attempt; the extra identity also observes the delay. Pass the same `--streams=N` to the target, whose independent developer connection budget includes the extra identity and one retiring/replacement overlap. The 4096 maximum bounds this test procedure only; it is not a production connection limit.
...
The harness's only command output is bounded JSON containing `phase`, `attempted`, `open`, `verified`, `closed`, and a fixed failure category. A successful topology finishes with totals `514 / 514 / 514 / 514` at the default size: 512 holder successes, one additional identity success, and one successful replacement. For `N` holder streams each final total must equal `N + 2`. Output never includes the token, identities, relay URL, target hostname, certificate data, destinations, or payloads. Preserve only the final aggregate harness result plus the sanitized resource observations defined below in the physical-acceptance record.
...
A passing run must use exactly two freshly provisioned authenticated Client identities. The first identity establishes and verifies all configured holder streams. The second identity makes one additional attempt, which must open and verify without evicting any holder stream. Any legacy `client_stream_limit` or `agent_stream_limit` rejection fails this run, while the decoder retains compatibility with these finite peer codes. After the hold, the runner closes one holder stream and opens and verifies one replacement. Default attempted/open/verified/closed totals must be `514/514/514/514` (`N+2` each for a custom size); both temporary identities must be revoked exactly once; and every session and successfully opened stream must close within the cleanup budget.
