...
- Pair only by scanning the short-lived Agent QR generated by an enrolled Owner in the Windows app. The decoded, immutable base64url Agent invitation must contain version 1, an HTTPS relay origin, one CA certificate, the `agent` role, a nonempty one-use capability, and an unexpired RFC 3339 expiry. The Android UI never renders, copies, or accepts a pasted invitation.
...
This checklist is still required for release acceptance; it is not performed by the automated build or test commands.
...
- [ ] From the owner Windows app, issue a short-lived Agent QR code. Keep it on the trusted local display; do not transfer its invitation text.
- [ ] Use **Scan QR** to pair once. Confirm the app reports only `paired` and never displays the invitation text.
