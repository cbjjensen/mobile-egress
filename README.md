...
2. Before opening `MobileEgressSetup.exe`, open the system **Windows PowerShell** from the Start menu—not a program or script from the ZIP—and run `Get-AuthenticodeSignature` on that exact setup path as shown in the [Windows friend quick start](windows-client/README.md#friend-quick-start). Continue only when the signer thumbprint is exactly `85F220C1BF05A5D3A86B5DD408787EC1B122ECB7`, Status is exactly `NotTrusted` on a fresh PC or `Valid` on an already-trusted PC, and the OS-extracted certificate SHA-256 matches the separately shared value. Reject `HashMismatch`, `NotSigned`, `UnknownError`, or any other status. Windows Properties alone and values shown by setup/the ZIP are insufficient. After this check, run setup, approve its explicit confirmation and one UAC prompt, and let it transactionally install and normally launch the controller.
