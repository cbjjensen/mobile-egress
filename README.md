...
6. In **AWS Login**, use IAM Identity Center browser login (recommended) or the encrypted access-key fallback. The controller is fixed to `us-east-1`.
   - If the friend only has the AWS root login, they may use root in the browser to enable IAM Identity Center. In the AWS page, choose **Enable**, choose **Single-Region instance** in **US East (N. Virginia)**, create a user for themselves, assign that user access to the AWS account, and copy the **AWS access portal URL** into Mobile Egress.
   - The URL pasted into Mobile Egress should look like `https://d-xxxxxxxxxx.awsapps.com/start` or another AWS access portal URL, not the normal EC2 console URL.
   - If Identity Center is unavailable, create an IAM user for Mobile Egress and use that IAM user's access key in the fallback. Do not use root access keys.
7. In **EC2 Nodes**, select up to ten running x86-64 Windows Server 2019 instances. Choose **Prepare SSM** where needed, wait for **SSM online**, then choose **Install Client**.
