...
2. Before the first run, compare the signer certificate Windows shows for the exact `MobileEgressSetup.exe` with the SHA-256 fingerprint received through the separate channel. You can inspect **Properties → Digital Signatures** or use trusted system Windows PowerShell as described in the [Windows friend quick start](windows-client/README.md#friend-quick-start). Then double-click setup, confirm the displayed fingerprint with **Yes**, approve one UAC prompt, and let it transactionally install and launch the controller.
