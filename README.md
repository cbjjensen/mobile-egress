...
2. Open system **Windows PowerShell** from the Start menu and copy the atomic verifier/launcher command from the separately shared [Windows friend quick start](windows-client/README.md#friend-quick-start)—never run a script from the ZIP or double-click setup. The command holds a mutation-denying handle while requiring Status `NotTrusted`/`Valid`, exact signer SHA-1 and certificate SHA-256, hashing the same held file, and calling `Start-Process` with `--verified-setup-sha256`. Setup rejects a missing/wrong digest, retains its own lock through child completion, and launches the installed controller only after child exit zero plus bound success.
